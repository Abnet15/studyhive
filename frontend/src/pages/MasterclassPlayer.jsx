...
import { Play, Pause, Sparkles, Video, X, SkipForward, RotateCcw, Volume2, CheckCircle, XCircle } from 'lucide-react';
...
// ─── Professor Avatar ─────────────────────────────────────────────────────────
const ProfessorAvatar = ({ isPlaying, mood = 'neutral' }) => {
  const faces = { neutral: '🧑‍🏫', correct: '🥳', wrong: '🤗', thinking: '🤔', speaking: '👨‍💼' };
  return (
    ...
    <div className="flex flex-col gap-3">
      {steps.map((s, i) => (
        <React.Fragment key={i}>
          <AnimatePresence>
            {i < rev && (
              <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ type: 'spring', stiffness: 200 }}
                ...
                <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 2, repeat: Infinity }}
                  className="w-12 h-12 rounded-xl bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center text-2xl shrink-0">{s.icon || '📌'}</motion.div>
          ...
          {i < steps.length - 1 && i < rev - 1 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-center">
              <motion.div animate={{ y: [0, 4, 0] }} transition={{ duration: 1, repeat: Infinity }} className="text-indigo-500 text-xl">↓</motion.div>
            </motion.div>
          )}
        </React.Fragment>
      ))}
...
const ComparisonAnimation = ({ left, right, isPlaying }) => {
  const [revL, setRevL] = useState(0); const [revR, setRevR] = useState(0);
  const t = useRef(null);
  ...
  const Col = ({ data, color, fromLeft, rev }) => (
    <motion.div initial={{ opacity: 0, x: fromLeft ? -60 : 60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, type: 'spring' }} className={`flex-1 rounded-2xl border p-5 space-y-3 ${color}`}>
      <div className="text-sm font-black uppercase tracking-widest text-white/80 mb-4">{data?.label}</div>
      {(data?.points || []).map((pt, i) => (<AnimatePresence key={i}>{i < rev && (<motion.div initial={{ opacity: 0, x: fromLeft ? -20 : 20 }} animate={{ opacity: 1, x: 0 }} className="flex items-start gap-2 text-sm text-slate-300"><span className="w-1.5 h-1.5 rounded-full bg-current mt-1.5 shrink-0" />{pt}</motion.div>)}</AnimatePresence>))}
    </motion.div>
  );
  return (
    <div className="flex gap-4">
      <Col data={left} color="bg-indigo-900/30 border-indigo-500/30" fromLeft rev={revL} />
      <div className="flex items-center"><div className="w-px self-stretch bg-white/10 relative"><motion.div animate={{ y: ['-20%', '80%', '-20%'] }} transition={{ duration: 3, repeat: Infinity }} className="absolute left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.8)]" /></div></div>
      <Col data={right} color="bg-fuchsia-900/30 border-fuchsia-500/30" fromLeft={false} rev={revR} />
    </div>
  );
};
...
const CodeAnimation = ({ code = '', isPlaying }) => {
  ...
  return (
    <div className="w-full rounded-2xl overflow-hidden border border-white/10 bg-[#0B1121]">
      <div className="flex items-center gap-1.5 px-4 py-3 bg-white/5 border-b border-white/5">
        ...
        <span className="ml-3 text-xs text-slate-600 font-mono">snippet.js</span>
        {isPlaying && <motion.div animate={{ opacity: [1, 0, 1] }} transition={{ duration: 0.8, repeat: Infinity }} className="ml-auto w-2 h-4 bg-emerald-400 rounded-sm" />}
      </div>
      <pre className="p-6 text-sm text-emerald-400 font-mono overflow-x-auto whitespace-pre-wrap leading-relaxed min-h-[100px]">{shown}{shown.length < code.length && isPlaying && <motion.span animate={{ opacity: [1, 0, 1] }} transition={{ duration: 0.5, repeat: Infinity }}>|</motion.span>}</pre>
    </div>
  );
};
...
const ConceptAnimation = ({ steps = [], icon, isPlaying }) => {
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ minHeight: '240px' }}>
      <motion.div animate={isPlaying ? { scale: [1, 1.08, 1], boxShadow: ['0 0 0px rgba(99,102,241,0)', '0 0 30px rgba(99,102,241,0.4)', '0 0 0px rgba(99,102,241,0)'] } : {}} transition={{ duration: 2.5, repeat: Infinity }}
        className="absolute w-20 h-20 rounded-full bg-indigo-500/20 border-2 border-indigo-500/50 flex items-center justify-center text-4xl z-10" style={{ left: '50%', top: '50%', transform: 'translate(-50%,-50%)' }}>
        {icon || '💡'}
      </motion.div>
      {steps.slice(0, 4).map((s, i) => (
        <AnimatePresence key={i}>{i < rev && (<motion.div initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: 'spring', stiffness: 200 }} className="absolute z-20 max-w-[120px] p-3 rounded-2xl bg-slate-800/80 border border-white/10 text-center shadow-lg" style={pos[i]}>
          <div className="text-xl mb-1">{s.icon || '✦'}</div>
          <div className="text-xs font-bold text-white leading-tight">{s.label}</div>
          {s.description && <div className="text-[10px] text-slate-500 mt-1">{s.description}</div>}
...
const InteractiveScene = ({ scene, onAnswer, selectedChoice, professorMood, isPlaying }) => {
  return (
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        ...
        <div className="text-xs font-black text-amber-400 uppercase tracking-widest mb-3 flex items-center gap-2">
          <span className="text-lg">🎯</span> QUICK CHECK — Test Your Understanding
        </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        {(scene.choices || []).map((choice, idx) => {
          ...
          let cardStyle = 'bg-white/4 border-white/10 hover:bg-indigo-500/10 hover:border-indigo-500/40 cursor-pointer';
          ...
          if (showResult && isSelected && choice.isCorrect) {
            cardStyle = 'bg-emerald-500/20 border-emerald-500/60 cursor-default';
            icon = <CheckCircle className="w-8 h-8 text-emerald-400 shrink-0" />;
          } else if (showResult && isSelected && !choice.isCorrect) {
            cardStyle = 'bg-red-500/20 border-red-500/60 cursor-default';
            icon = <XCircle className="w-8 h-8 text-red-400 shrink-0" />;
          } else if (showResult && choice.isCorrect) {
            cardStyle = 'bg-emerald-500/10 border-emerald-500/30 cursor-default opacity-70';
            icon = <CheckCircle className="w-8 h-8 text-emerald-400/60 shrink-0" />;
          } else if (showResult) {
            cardStyle = 'bg-white/2 border-white/5 cursor-default opacity-40';
          }

          return (
            <motion.button key={idx}
              initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + idx * 0.1 }}
              whileHover={!selectedChoice ? { x: 6 } : {}}
              onClick={() => !selectedChoice && onAnswer(choice)}
              className={`flex items-center gap-4 p-4 rounded-2xl border text-left transition-all ${cardStyle}`}
            >
              {icon}
              <span className="text-sm font-medium text-white leading-snug">{choice.text}</span>
            </motion.button>
          );
        })}
      </div>

      {/* Teacher response after answer */}
      <AnimatePresence>
        {selectedChoice && (
          <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 0.2 }}
            className={`p-5 rounded-3xl border flex items-start gap-4 ${selectedChoice.isCorrect ? 'bg-emerald-900/30 border-emerald-500/30' : 'bg-slate-800/60 border-white/10'}`}>
            <span className="text-3xl shrink-0">{selectedChoice.isCorrect ? '🥳' : '🤗'}</span>
            <div>
              <div className={`text-xs font-black uppercase tracking-widest mb-1 ${selectedChoice.isCorrect ? 'text-emerald-400' : 'text-amber-400'}`}>
                {selectedChoice.isCorrect ? '✓ Correct! Professor says:' : 'Professor says:'}
              </div>
              <p className="text-sm text-slate-200 leading-relaxed font-medium">{selectedChoice.teacherResponse}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
const SceneVisual = ({ scene, isPlaying }) => {
  ...
  switch (type) {
    ...
    case 'concept': default: return <ConceptAnimation steps={scene.visualSteps || []} icon={scene.icon} isPlaying={isPlaying} />;
  }
};
...
const MasterclassPlayer = () => {
  ...
  const playScene = useCallback((index) => {
    ...
    // Speak the intro script
    speakText(scene.teacherScript, () => {
      clearInterval(progressRef.current);
      setProgress(100);
      
      // If this is an interactive scene, pause and wait for student answer
      if (scene.type === 'interactive') {
        setProfessorMood('thinking');
        setWaitingForAnswer(true);
        setIsPlaying(false);
      } else {
        // Auto-advance to next scene after a short pause
  ...
  const handleAnswer = useCallback((choice) => {
    ...
    setWaitingForAnswer(false);
    setProfessorMood(choice.isCorrect ? 'correct' : 'wrong');

    // Teacher speaks the personalized response
    speakText(choice.teacherResponse, () => {
      setProfessorMood('neutral');
      // Move to next scene after a moment
      setTimeout(() => {
        if (currentSlide + 1 < (data?.scenes?.length || 0)) playScene(currentSlide + 1);
      }, 1200);
    });
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center text-white gap-8">
      ...
      <motion.h2 animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 2, repeat: Infinity }} className="text-2xl font-black uppercase tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400 text-center">Preparing Your Virtual Professor...</motion.h2>
      <div className="flex gap-3">
        {['Analyzing', 'Scripting', 'Designing Q&A'].map((s, i) => (
          <motion.div key={i} animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 1.5, delay: i * 0.5, repeat: Infinity }} className="px-4 py-2 rounded-full bg-white/4 border border-white/10 text-slate-500 text-xs font-bold">{s}</motion.div>
        ))}
      </div>
    </div>
  );
  ...
  if (error || !data?.scenes?.length) return (
    <div className="min-h-screen bg-[#020617] flex items-center justify-center text-white">
      <div className="text-center space-y-4"><div className="text-6xl">💔</div><h2 className="text-2xl font-bold">Assembly Failed</h2><p className="text-red-400">{error || 'AI could not structure this lesson.'}</p>
        <button onClick={() => navigate(-1)} className="mt-6 px-8 py-3 rounded-2xl bg-indigo-600 hover:bg-indigo-500 font-bold transition-colors">Go Back</button></div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#020617] text-white flex flex-col overflow-hidden relative">
          ...
          <div className="flex gap-1">
            {data.scenes.map((s, idx) => (
              <button key={idx} onClick={() => { stopAll(); setTimeout(() => playScene(idx), 80); }}
                className={`h-1.5 rounded-full transition-all duration-300 ${idx === currentSlide ? (s.type === 'interactive' ? 'w-8 bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.9)]' : 'w-8 bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.9)]') : 'w-1.5 bg-slate-800 hover:bg-slate-600'}`} />
            ))}
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden">
            ...
            >
              ...
              <div className={`px-8 py-5 border-b border-white/5 flex items-center gap-4 ${isInteractive ? 'bg-amber-500/5' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className={`text-[10px] font-black uppercase tracking-widest mb-0.5 ${isInteractive ? 'text-amber-400' : 'text-indigo-400'}`}>
                    Scene {currentSlide + 1} · {isInteractive ? '🎯 INTERACTIVE CHECK' : (scene.animationType || 'concept').toUpperCase()}
                  </div>
                ...
                {waitingForAnswer && (
                  <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.7, 1, 0.7] }} transition={{ duration: 1.5, repeat: Infinity }}
                    className="px-3 py-1.5 rounded-xl bg-amber-500/20 border border-amber-500/40 text-amber-300 text-xs font-black">
                    YOUR TURN
                  </motion.div>
                )}
              </div>
              ...
              <div className="p-8 md:p-10">
                {isInteractive ? (
                  <InteractiveScene scene={scene} onAnswer={handleAnswer} selectedChoice={selectedChoice} professorMood={professorMood} isPlaying={isPlaying} />
                ) : (
              ...
              {/* Bottom accent */}
              <motion.div className={`h-[2px] bg-gradient-to-r ${isInteractive ? 'from-amber-500 via-orange-500 to-red-500' : 'from-indigo-500 via-purple-500 to-cyan-500'}`}
                initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 0.4, duration: 0.8 }} style={{ transformOrigin: 'left' }} />
            </motion.div>
          </AnimatePresence>

          {/* Karaoke */}
          <div>
            <div className="text-[10px] text-slate-600 font-black uppercase tracking-widest mb-2 flex items-center gap-2"><Volume2 className="w-3 h-3" /> Professor Live Script</div>
        ...
        <div className="w-full lg:w-80 xl:w-96 flex flex-col gap-4 p-6 border-t lg:border-t-0 lg:border-l border-white/5 bg-black/20 backdrop-blur-3xl shrink-0">
          ...
          {/* Professor avatar + controls */}
          <div className="p-7 rounded-3xl bg-white/3 border border-white/8 flex flex-col items-center gap-5">
            <div className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Virtual Professor</div>
            <ProfessorAvatar isPlaying={isPlaying} mood={professorMood} />

            {waitingForAnswer ? (
              <motion.div animate={{ opacity: [0.7, 1, 0.7] }} transition={{ duration: 1.5, repeat: Infinity }}
                className="w-full py-3 rounded-2xl bg-amber-500/20 border border-amber-500/30 text-amber-300 text-sm font-bold text-center">
              ...
              <div className="relative">
                ...
                <button onClick={() => { if (isPlaying) { synthRef.current?.pause(); setIsPlaying(false); } else { if (synthRef.current?.paused) { synthRef.current.resume(); setIsPlaying(true); } else playScene(currentSlide); } }}
                  ...
                  <AnimatePresence mode="wait">
                    {isPlaying ? <motion.div key="p" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}><Pause className="w-8 h-8" /></motion.div>
            ...
            <div className="flex gap-2 w-full">
              ...
              <button onClick={() => { stopAll(); if (currentSlide < totalSlides - 1) setTimeout(() => playScene(currentSlide + 1), 80); }} disabled={currentSlide >= totalSlides - 1}
                className="flex-1 py-2.5 bg-white/5 hover:bg-white/10 disabled:opacity-30 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 text-slate-300 transition-colors"><SkipForward className="w-3.5 h-3.5" /> Skip</button>
            </div>

            {/* Scene bar */}
            <div className="w-full space-y-1">
              <div className="flex justify-between text-[10px] text-slate-700"><span>Progress</span><span>{Math.round(progress)}%</span></div>
